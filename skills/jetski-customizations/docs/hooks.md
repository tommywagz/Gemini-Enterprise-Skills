...
specific points during the Jetski Agent's execution loop. This is powerful for
...
root directory (e.g., `_agents/hooks.json` in google3).
...
:                  :                   :                  : `hooks` wrapper). :
| `PostToolUse`    | After a tool step | Tool name        | Grouped (uses     |
:                  : completes.        : (Currently       : `matcher` &       :
:                  :                   : ignored, see     : `hooks` wrapper). :
:                  :                   : limitations).    :                   :
...
```json
{
  ...
  "workspacePaths": ["/google/src/cloud/username/workspace/google3"],
  "transcriptPath": "/google/src/cloud/username/workspace/google3/.gemini/jetski/transcript.jsonl",
  "artifactDirectoryPath": "/google/src/cloud/username/workspace/google3/.gemini/jetski/artifacts",
  "modelName": "auto",
  "isBattleMode": false
}
```
...
Use for post-execution cleanup, auto-fixes, analysis, or to rewrite what the
agent sees.
...
*   **Output (stdout)**: An empty JSON object `{}`, or:

    ```json
    {
      "overwriteResult": "[redacted by security policy]"
    }
    ```

    *   **`overwriteResult`** (string, optional): Replaces the result of the
        tool call that just ran. Omit to leave it untouched; set to `""` to
        blank it out. When several `PostToolUse` hooks match the same tool call,
        each hook sees the current (possibly already replaced) result and the
        last one to set the field wins.
...
*   **`PostToolUse` does not currently support the `matcher` field** (it runs
    for all tools).
