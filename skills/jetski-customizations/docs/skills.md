...
customization root (e.g., `_agents/skills/` in google3).
...
1.  Run the preparation script:
    [prepare.sh](file:///google3/path/to/your/skill/scripts/prepare.sh)
2.  Execute the test command:
    `blaze test //path/to:xyz_integration_test`
...
2.  **Executable Helpers**: Encapsulate complex command sequences in scripts
    within the `scripts/` directory. Link to them using clickable `file://`
    links so the agent can easily find and run them.
...
## Google3 Contribution and Validation

If you are contributing skills to a shared team repository (such as
`//learning/gemini/agents/skills/`), you must define a `BUILD` file in the skill
directory to enable automated structure validation.

Use the `agent_skill` rule:

```bazel
load("//learning/gemini/agents/skills:build_defs.bzl", "agent_skill")

agent_skill(
    name = "my_specialized_skill",
    srcs = ["SKILL.md"],
