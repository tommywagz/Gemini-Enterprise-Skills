...
Jetski, configuring MCP servers allows you to expose custom tools, resources,
...
Jetski supports two transport mechanisms for MCP: **Stdio** (for local
...
1.  **Tool Discovery**: Jetski queries the server for its available tools.
...
*   **Plugin Servers**: Only active when the parent plugin is loaded. Their
    ...
    conflicts.
*   You can inspect active MCP servers and their tools in the Jetski UI by
    navigating to **Additional Options (...) > MCP Servers**.
