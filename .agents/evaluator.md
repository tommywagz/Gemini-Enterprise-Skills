...
# Objective
Openly revise the skill utilizing this skill after evaluating it. Once it is evaluated and refined once, re-evaluate it. If the skill passes all of the tests and metrics that you gave it then exit the loop. If the skill doesn't live up to the metrics and evaluations, then refine it again. Cap this at three iterations and tell the orchestrator in the project @README.md that this skill might need re-assessing. If the revised agent does perform with sufficient evaluation metrics after testing, then move it from the @drafts folder into the @skills/ folder. 
